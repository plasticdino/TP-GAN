...
class MultiPIE():
    ...
    def __init__(self,params, datasplit='train',
                 Random=True,
                 LOAD_60_LABEL=False,
        ...
        self.csvpath = params.csvpath
        self.feat5ptDir = params.feat5ptDir
        ...
        self.cameraPositions = {'24_0': (+90, '10'), '01_0': (+75, '08'), '20_0': (+60, '08'), '19_0': (+45, '09'),
                                '04_1': (+30, '07'), '05_0': (+15, '06'),  # left
                                '05_1': (0, '06'),  # center
                                '14_0': (-15, '06'), '13_0': (-30, '05'), '08_0': (-45, '15'), '09_0': (-60, '15'),
                                '12_0': (-75, '15'), '11_0': (-90, '15')}  # right

        if not testing:
            split_f = self.csvpath.format(source, self.split)  # data/train/train.csv
            split_f_test = self.csvpath.format('test', 'test')  # data/test/test.csv
    ...
    def test_batch(self, test_batch_size=4, Random=True, Pose=-1):
        ...
        for i in range(test_batch_size):
            ...
            # if Pose != -1: #Pose is now 60
            #     while True:
            #         pose = abs(self.findPose(self.indices_test[j % len(self.indices_test)]))
            #         print("j = ",j , "indices_test = ", len(self.indices_test))
            #         if pose == Pose:
            #             break
            #         j += 1
            ...
            if not self.testing:
                labels[i, ...], _, leyel[i, ...], leyer[i, ...], lnose[i, ...], lmouth[i, ...] = self.load_label_mask(
                    '/test/test_img/' + filename)
    ...
    def next_image_and_label_mask_batch(self, batch_size, imageRange=-1, imageRangeLow=0, labelnum=None):
        ...
        for i in range(batch_size):
            if imageRange != -1:
                ...
                #             break
                self.updateidx()  # somehow it's always 0 :)

            images[i, ...], feats = self.load_image('/train/train_img/' + self.indices[self.idx])
            filename = self.indices[self.idx]
            labels[i, ...], _, leyel[i, ...], leyer[i, ...], lnose[i, ...], lmouth[i, ...] = self.load_label_mask(
                '/train/train_img/' + filename)
    ...
    def load_label_mask(self, filename, labelnum=-1):
        ...
        _, labelname = self.findSameIllumCodeLabelpath(filename)  # got label path here: 001_01_01_051_02_crop_128.png
    ...
    def findBestIllumCodeImagepath(self, fullpath):
        print("people say this never been used")
        ...
        codepath = str(labelpath).replace('crop_128', 'code')
    ...
    def findSameIllumCodeLabelpath(self, fullpath):
        # change name of image into 05_1 pose: 0 degree
        span = re_poseIllum.search(fullpath).span()
        tempath = list(
            fullpath)  # tempath  =  ['0', '0', '1', '_', '0', '1', '_', '0', '1', '_', '0', '5', '1', '_', '0', '2', '_', 'c', 'r', 'o', 'p', '_', '1', '2', '8', '.', 'p', 'n', 'g']
        if self.LOAD_60_LABEL:
        ...
        labelpath = ''.join(tempath)  # labelpath 001_01_01_051_02_crop_128.png
        codepath = str(labelpath).replace('crop_128', 'code')  # codepath 001_01_01_051_02_code.png not been used
        return (codepath, labelpath)

    def findPose(self, fullpath):
        # find camerapostion
        span = re_pose.search(fullpath).span()  # 001_01_01_051_02_crop_128.png span = (9,14)
        camPos = list(fullpath[span[0] + 1:span[1] - 1])
        camPos.insert(2, '_')
        camPos = ''.join(camPos)  # 08_0
        # get 01_0 like string
        return self.cameraPositions[camPos][0]  # -45
    ...
    def GetFeatureParts(self, img_resize, filename, label=False):
        ...
        if True:  # not label:
            featpath = self.feat5ptDir + filename[0:-16] + '.5pt'
            featpath = featpath.replace("train_img", "keypoint")
            featpath = featpath.replace("test_img", "keypoint")
            # print(filename) data/test/test_img/001_01_01_010_04_crop_128.5pt
        ...
        # print('featpath=', featpath) #data/test/test_img/001_01_01_051.5pt
        # print('trans_points=', trans_points) #[[295 194] [369 187] [338 229] [312 275] [368 269]]
        # print('file ', filename, '\ncrop_y_end:cropy=', crop_y_end, ':', crop_y) #/test/test_img/001_01_01_051_05_crop_128.png  128 : 88
        # print('crop_x_end:crop_x=', crop_x_end, ':', crop_x) #128 : 88
        # print('img_resize shape=', img_resize.shape) #(128, 128, 3)
        eyel_crop[...] = img_resize[crop_y:crop_y_end, crop_x:crop_x_end, :];
        ...
        # print("430 pos_s = filename[-19:-16] = ", pos_s)
